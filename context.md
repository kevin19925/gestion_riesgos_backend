...
| **Proceso** | Unidad de análisis: nombre único, estado (`borrador`, etc.), área, gerencia, responsable, documentos, participantes, **ProcesoResponsable** (varios usuarios con `modo`: dueño/supervisor). **`residualModo`:** `ESTANDAR` (mitigación % estándar) o `ESTRATEGICO` (criterios MA / motor CWR en `estrategicoResidual.*`). |
